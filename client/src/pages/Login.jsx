  ...
  const navigate = useNavigate(); 
    const onFinish = async (values) => {
      try {
        ...
        const response = await axios.post("http://localhost:12000/api/auth/login",values);
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <Form layout='vertical' onFinish={onFinish} className='flex flex-col gap-4'>
        <Form.Item  name='email'  className='border p-1 rounded-lg focus:outline-none focus:border-blue-500'>
          <input
            type='email'
